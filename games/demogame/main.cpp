		...
		else
		{
			...
			while( !quit )
			{
               ...
               if( Mix_PlayingMusic() == 0 )
                ...
                if (!gameOver) {
                        gScoreTextTexture.render(900,0,NULL, gRenderer);
                gHightScoreTextTexture.render(900, 50, NULL, gRenderer);
                }
                if (gameOver) {
                GameOverTexture.render(-5, -5, NULL, gRenderer);
    ...
    DinosourTexture.loadFromFile( "img//char.png", gRenderer );
{
	...
	gFontGameOver = TTF_OpenFont("lazy.ttf", 60);
